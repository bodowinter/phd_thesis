...
##------------------------------------------------------------------
## Analysis of variability by SentiWordNet 3.0 synsets:
##------------------------------------------------------------------

## For all adjectives, calculate valence variabiltya cross synsets:
...
for (i in 1:nrow(xall)) {
	...
	if (length(these_sent) > 0) {
		...
		xall[i,]$PosScoreSD <- sds[1]
		xall[i,]$NegScoreSD <- sds[2]
...
xall$ValSDMean <- rowMeans(xall[,c('PosScoreSD', 'NegScoreSD')])

## Model:

summary(xmdl.sd <- lm(ValSDMean ~ DominantModality, xall))
anova(xmdl.sd)

## Get predictions and re-order them:

sd.pred <- my.predict.lm(xmdl.sd)
sd.pred$DominantModality <- factor(sd.pred$DominantModality, levels = modalities)
sd.pred <- sd.pred[order(sd.pred$DominantModality), ]

## Plot of SynSet emotional variability:

setup_plots(N = 1)
emptyplot(xlim = c(0.5, 5.5), ylim = c(0.08, 0.18))
draw_preds(sd.pred)
lower_axis(N = xall$DominantModality, type = 3)
top_labels(first_text = 'Valence Variability (SentiWordNet 3.0)', second_text = '', type = 2)
left_axis(text = 'Sense Valence SD', at = seq(0.08, 0.18, 0.02), type = 1)
